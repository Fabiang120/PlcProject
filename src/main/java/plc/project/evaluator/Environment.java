...
public final class Environment {
    ...
    private static RuntimeValue range(List<RuntimeValue> arguments) throws EvaluateException {
        if (arguments.size() != 2) {
            throw new EvaluateException("range expects 2 arguments (start, end)", Optional.empty());
        }
        if (!(arguments.get(0) instanceof RuntimeValue.Primitive startPrim) ||
            !(arguments.get(1) instanceof RuntimeValue.Primitive endPrim) ||
            !(startPrim.value() instanceof java.math.BigInteger start) ||
            !(endPrim.value() instanceof java.math.BigInteger end)) {
            throw new EvaluateException("range arguments must be integers (BigInteger)", Optional.empty());
        }
        if(start.compareTo(end)>0){
            throw new EvaluateException("range start must be <= end", Optional.empty());
        }
        java.util.List<RuntimeValue> list = new java.util.ArrayList<>();
